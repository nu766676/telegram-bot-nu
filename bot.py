...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'about':
        ...
        await query.message.reply_photo(photo=FILE_URLS["bron"], caption="Для брони, пожалуйста, позвоните: +79148985744", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Назад ↩️", callback_data='back')]]))
    elif data == 'menu':
        await query.message.reply_text("Нажмите на кнопку ниже, чтобы открыть меню:", reply_markup=InlineKeyboardMarkup([
        ...
        await query.message.reply_photo(photo=FILE_URLS["anketa"], caption="Заполни анкету и получи бонусы! 💰", reply_markup=InlineKeyboardMarkup([
...
def webhook():
    ...
    # Используем уже активный event loop
